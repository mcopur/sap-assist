...
class Chatbot:
    def __init__(self, intent_model_path, response_model_path):
        ...
        self.response_model = BertForMaskedLM.from_pretrained(response_model_path).to(self.device)
    ...
    def generate_response(self, intent, entities, context):
        ...
        inputs = self.response_tokenizer(prompt, return_tensors="pt", truncation=True, padding=True).to(self.device)
